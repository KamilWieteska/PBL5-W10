package com.iot.workshop.devicesimulator.aws.utils;
...
public class CommandLineUtils {
    ...
    public MqttClientConnection buildWebsocketX509MQTTConnection( MqttClientConnectionEvents callbacks ) {
        try {
            ...
            TlsContextOptions x509TlsOptions =
                    TlsContextOptions.createWithMtlsFromPath( getCommandRequired( m_cmd_x509_cert_file, "" ),
                                                              getCommandRequired( m_cmd_x509_key_file, "" ) );
    ...
    public MqttClientConnection buildDirectMQTTConnection( MqttClientConnectionEvents callbacks ) {
        try {
            ...
            System.out.println( "Error while creating the connection: " + ex.getMessage() );
            return null;
        }
    }
    ...
    private void buildConnectionSetupConnectionDefaults( AwsIotMqttConnectionBuilder builder, MqttClientConnectionEvents callbacks ) {
        builder.withConnectionEventCallbacks( callbacks )
                .withClientId( getCommandOrDefault( "client_id", "test-" + UUID.randomUUID()
                        .toString() ) )
